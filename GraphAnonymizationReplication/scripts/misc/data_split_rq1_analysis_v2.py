...
if __name__ == "__main__":
    ...
    #             file_name = \
    #                 os.path.basename(__file).replace("final_", "").replace(__d + "_", "").replace("_results", "").replace(
    #                     "_clean.xlsx", "").split(" ")[0]
    #
    #
    ...
    #             clean_df[f"{__p}_cumulative_gain"] = clean_df[f"{__p}_cumulative_gain"].apply(lambda __x: max_value - __x)
    ...
    #             clean_df[f"{__p}_cumulative_gain"] = clean_df.apply(lambda __x: __x[f"{__p}_cumulative_gain"]+ " - " + str(__x["iteration"]), axis=1 )
    ...
    #                             label=file_name, marker='o', color=color)
    ...
    #
    #
    #         plt.text(18, 60, "Privacy Level I", ha='center', va='bottom', fontsize=fontsize_privacy, rotation=0)
    #         plt.plot([0, 19], [80, 80], color='black', linestyle='dotted', linewidth=1)
    #         plt.text(18, 80, "Privacy Level II", ha='center', va='bottom', fontsize=fontsize_privacy, rotation=0)
    #         plt.plot([0, 19], [90, 90], color='black', linestyle='dotted', linewidth=1)
    #         plt.text(18, 90, "Privacy Level III", ha='center', va='bottom', fontsize=fontsize_privacy, rotation=0)
    #
    #         plt.plot([0, 19], [100, 100], color='red', linestyle='--', linewidth=1)
    ...
    #         plt.savefig(f"stat_test/{rq}/{rq}_{__d}_{__p}.png")
    #         plt.close()
    #     # plt.show()
    ...
    for __t in technique_name:
        ...
        size = 0
        # for __p in ["privacy_measure_tim_ipr", "privacy_measure_found_percentage"]:
        for __p in ["privacy_measure_found_percentage"]:
            plt.clf()
            ...
            for __file in file_list:
                ...
                # clean_df = pd.read_excel(__file)
                clean_df = pd.read_excel(__file)
                # if file_name in ["qt", "groovy", "camel"]:
                #     continue

                clean_df[__p] = clean_df[__p].astype(float)
                clean_df[__p] = clean_df[__p] * 100

                clean_df["privacy_measure_tim_ipr"] = clean_df["privacy_measure_tim_ipr"].astype(float)
                clean_df["privacy_measure_tim_ipr"] = clean_df["privacy_measure_tim_ipr"] * 100
                ...
                sns.lineplot(x=xaxis, y=__p, data=clean_df, markers=True, dashes=False,
                             label=file_name, marker='o', color=color)
                # make line style dotted
                sns.lineplot(x=xaxis, y="privacy_measure_tim_ipr", data=clean_df, markers=True, dashes=False,
                                label=file_name, marker='^', color=color, linestyle='dotted')
            ...
            plt.plot([0, size], [100, 100], color='red', linestyle='dotted', linewidth=1)
            # plt.xticks(range(0, size, 1))

            # plt.title(title)
            # remove some elemts from the legend
            handles, labels = plt.gca().get_legend_handles_labels()
            # remove every second element
            handles = handles[::2]
            labels = labels[::2]
            plt.legend(handles=handles, labels=labels, loc='lower right', fontsize=font_size)
            ...
            if __t == "random_add_delete":
                ...
                title = "Genaralisation"
                xticks = np.arange(350, 525, 25)
                xlim = 325, 525
                start = 500

            plt.text(start, 60, "Privacy Level I", ha='center', va='bottom', fontsize=font_size, rotation=0)
            plt.plot([0, size], [80, 80], color='black', linestyle='dotted', linewidth=1)
            plt.text(start, 80, "Privacy Level II", ha='center', va='bottom', fontsize=font_size, rotation=0)
            plt.plot([0, size], [90, 90], color='black', linestyle='dotted', linewidth=1)
            plt.text(start, 90, "Privacy Level III", ha='center', va='bottom', fontsize=font_size, rotation=0)

            # print(plt.rcParams.keys())

            plt.title(f"{title}")
            plt.ylim(0, 105)
            plt.yticks(np.arange(0, 105, 10), fontsize=font_size)
            ...
            plt.ylabel("Privacy Score", fontsize=font_size)

            plt.xlabel("Configuration Level", fontsize=font_size)
            # plt.ylabel(get_label(__p), fontsize=font_size)
            # plt.legend()
            ...
            # plt.rcParams.update({'font.size': 30})
            plt.savefig(f"stat_test/{rq}/{rq}_{__t}_{__p}.png")
            plt.close()
            # plt.show()
    ...
    #                          label=file_name, marker='o', color=color, linestyle='None')
    ...
    #             plt.plot([0, maxx], [100, 100], color='black', linestyle='dotted', linewidth=1)
    #             plt.text(xstart, 60, "Privacy Level I", ha='center', va='bottom', fontsize=fontsize_privacy, rotation=0)
    #             plt.plot([100, maxx], [80, 80], color='black', linestyle='dotted', linewidth=1)
    #             plt.text(xstart, 80, "Privacy Level II", ha='center', va='bottom', fontsize=fontsize_privacy, rotation=0)
    #             plt.plot([100, maxx], [90, 90], color='black', linestyle='dotted', linewidth=1)
    #             plt.text(xstart, 90, "Privacy Level III", ha='center', va='bottom', fontsize=fontsize_privacy, rotation=0)
    #
    #         if "rf_results" in __p:
    #             # plt.plot([0, maxx], [baseline_performance, baseline_performance], color='black', linestyle='dotted', linewidth=1)
    #             # for __limit in [800, 90]:
    #             #     __limit = baseline_performance * __limit
    #             #     plt.plot([0, maxx], [__limit, __limit], color='black', linestyle='dotted', linewidth=1)
    #             start = clean_df["links_changed"].max()
    #             for __auc_limits, __auc_label in [(1, "Baseline"), (0.91, "90% Baseline"), (0.81, "80% Baseline"), ]:
    ...
    #                 plt.text(xstart, lower_bound_baseline*1.005, __auc_label, ha='right', va='center',
    ...
    #
    #         # plt.rcParams.update({'font.size': 30})
    #         plt.savefig(f"stat_test/{rq}/{rq}_links_{__t}_{__p}.png")
    #         plt.close()
    #         # plt.close('all')
